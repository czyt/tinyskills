...
Build language-neutral LazyCat LPK apps that expose a LightOS WebShell provider. Treat lightos-admin as the discovery, launcher, account boundary, and return surface; treat Go examples only as implementation references.
...
| Session restore | Use stable selector/tab/pane IDs, restore workspace state before attaching streams, replay history with identity markers |
...
| Mobile terminal | Safe-area layout, visual viewport, touch shortcuts, IME-safe input, no hover-only controls |
...
5. Make the browser terminal usable on touch devices:
   ...
   - Input: touch shortcuts for Tab/Return/arrows/Esc/modifiers/copy/paste/search/tab actions; no hover or hardware-keyboard dependency.
   - IME and gestures: composition-safe text flow, soft-keyboard resize, long-press selection handles, action sheets, tab overview, and browser-back guard.
